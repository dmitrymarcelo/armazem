...
export const ModuleSelector: React.FC<ModuleSelectorProps> = ({ user, onSelectModule, onLogout }) => {
  const modules: { id: SystemModule; name: string; description: string; icon: string; color: string; image: string }[] = [
    {
      id: 'warehouse',
      name: 'Armazém',
      description: 'Gestão de estoque, recebimento, expedição e inventário',
    ...
    },
    {
      id: 'workshop',
      name: 'Oficina',
      description: 'Ordens de serviço, manutenção preventiva e gestão de mecânicos',
        ...
        <div className="max-w-5xl w-full">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto">
              Escolha qual ambiente deseja acessar para gerenciar suas operações
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {modules.map((module) => (
              <button
                ...
                className="group relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 shadow-lg hover:shadow-2xl transition-all duration-300 border border-slate-200 dark:border-slate-700 text-left"
              >
                {/* Background Gradient */}
                ...
                <div className="relative p-8">
                  {/* Icon */}
                  <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${module.color} flex items-center justify-center shadow-lg mb-6 group-hover:scale-110 transition-transform duration-300`}>
          ...
          <div className="mt-12 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400">
              © 2024 LogiWMS Pro - Todos os direitos reservados
            </p>
          </div>
        </div>
      </main>
